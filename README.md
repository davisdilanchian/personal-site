...
Static site (no build step, no framework) hosted on **GitHub Pages** at the apex domain
**davisdilanchian.com**. The home page (`index.html`) is the dossier index; each project
also has its own page at `/<slug>/`.
...
```
index.html                home / master dossier (cards link to each /<slug>/)
assets/dossier.css        ONE shared stylesheet for every page
<slug>/index.html         per-project "case file" — has an editable Overview section
<slug>/privacy.html       privacy policy (App Store apps: pareto, mugula)
<slug>/support.html       support page    (App Store apps: pareto, mugula)
```

- **Describe a project:** open `<slug>/index.html` and edit the block under
  `▌ Overview` (marked with an EDIT comment). Plain HTML — `<p>`, `<h3>`, `<ul>`.
- **App Store URLs:** Apple wants a Privacy Policy URL and a Support URL per app, e.g.
  `https://davisdilanchian.com/pareto/privacy.html` and `.../pareto/support.html`.
  The Pareto/Mugula privacy + support pages are drafted — review and paste your final
  content into the `<div class="prose">` body.
- **Scaffold new project pages:** the detail pages were generated by a one-off script;
  to add another, copy an existing `<slug>/index.html` (it won't be overwritten).
...
{
  name: "Project Name",
  slug: "project-name",               // folder name -> links to /project-name/
